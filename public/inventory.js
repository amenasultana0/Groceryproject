...
window.reloadDataInBothTabs = async function () {
  ...
  form.addEventListener("submit", (e) => {
    ...
    row.querySelector(".delete-btn").addEventListener("click", () => {
      row.remove();
      updateSerialNumbers();
      notifyDataChange();  // SYNC UPDATE: notify after delete
    });
    ...
    notifyDataChange();  // SYNC UPDATE: notify after add
  });
  ...
  // SYNC UPDATE: notify other tabs that data changed
  function notifyDataChange() {
    localStorage.setItem('itemsUpdated', Date.now());
  }

  // SYNC UPDATE: listen for changes in other tabs and reload inventory
  window.addEventListener('storage', (event) => {
    if (event.key === 'itemsUpdated') {
      location.reload(); // reload page to sync data or call a function to reload items if applicable
    }
  });
});*/